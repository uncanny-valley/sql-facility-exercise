...
SELECT COUNT(facid) as facility_no_member_fee_count
FROM `Facilities`
WHERE membercost = 0.0;
...
SELECT `name`, CASE WHEN `monthlymaintenance` > 100 THEN "expensive"
					ELSE "cheap" END as monthly_maintenance_label
FROM `Facilities`;
...
SELECT DISTINCT f.name, m.firstname || ' ' || m.surname as member_name
FROM `Members` as m
...
SELECT f.name as facility, m.firstname || ' ' || m.surname as member_name,
    ...
    ELSE f.membercost END as total_cost
FROM `Bookings` as b
INNER JOIN `Facilities` as f ON b.facid = f.facid
...
FROM (SELECT * FROM `Bookings`
      WHERE `starttime` BETWEEN '2012-09-14 00:00:00' AND '2012-09-14 23:59:59') as b
INNER JOIN `Facilities` as f ON b.facid = f.facid
INNER JOIN `Members` as m on b.memid = m.memid
